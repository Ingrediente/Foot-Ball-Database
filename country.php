...
$str="'".$str."'";
$sql = "SELECT * FROM player WHERE country=".$str." order by fname";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
	   ...
	   {
		   echo "<tr class='even'>";
		echo "<td>".$row["player_id"]."</td><td>".$row["fname"]." ".
			$row["lname"]."</td>"."<td><form method='post' action='t.php'><input type='submit' name='f' value="."'".$row["team_name"]."'></input></form></td><td>".$row["salary"]."</td><td>".$row["age"]."</td><td>".$row["country"]."</td><td><form method='post' action='position.php'><input type='submit' name='position' value="."'".$row["position"]."'></input></form></td></tr>";
